-- ⚠️ 이 마이그레이션은 잘못된 테이블에 적용되었음.
--
-- 웹 전용 테이블인 줄 알고 public.subscriptions 를 변경했으나, 이 테이블은
-- Sniff 앱 구독 게이트가 이미 쓰던 테이블이었다. 컬럼 5개와 인덱스 1개가
-- 앱 테이블에 추가된 상태이며, 함께 걸었던 status CHECK 제약은 앱 구독 흐름을
-- 깨뜨릴 수 있어 수동으로 드롭한 뒤 이 파일에서도 제거했다.
--
-- 웹 정기결제는 0003_web_subscriptions.sql 로 분리했고,
-- 여기서 생긴 오염 제거는 0004_revert_subscriptions_pollution.sql 에 있다(실행 보류).
